...
// `acktypes.AWSResourceManagerFactory` interface.
type resourceManagerFactory struct {
	sync.RWMutex
	// rmCache contains resource managers for a particular AWS account ID and region
	rmCache map[string]*resourceManager
}

// ResourceDescriptor returns an AWSResourceDescriptor object that can be used
// by the upstream controller-runtime to introspect the CRs that the resource
// manager will manage as well as produce Kubernetes runtime object prototypes
