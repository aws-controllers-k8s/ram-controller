...
// resource implements the `acktypes.AWSResource` interface
type resource struct {
	// ko represents the Kubernetes object for the ResourceShareAccepter
...
// SetConditions sets the ACK Conditions collection for the AWSResource
func (r *resource) SetConditions(conditions []*ackv1alpha1.Condition) {
...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.ShareARN = &identifier.NameOrID
	return nil
}

// PopulateResourceFromAnnotation populates the fields passed from adoption annotation
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	shareARN, ok := fields["shareARN"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: shareARN"))
	}
	r.ko.Spec.ShareARN = &shareARN
	return nil
}

// ReplaceConditions replaces the resource's conditions collection entirely
